...
% --------------- Below is the code for Differenced RSI Trading stradegy -----------------
...
for sim = 1:n
    ...
    for iter = 1:40
        ...
        index = index + 30;
        if iter == 1 
            continue                                    % because we start from 2015.01.01, we need the algo to start from 30 days later
        end
        ...
        for stock = 1:95                                % 95 stocks in total
            ...
            upper = prctile(diff,100-var);             
            lower = prctile(diff,var);
...
xlabel('period')
ylabel('return')
title('Differenced RSI Trading - monthly returns for i-th period')
hold off
legend show
...
hold on
title('Differenced RSI Trading- cumulative returns until i-th period')
xlabel('period')
